# Arquivo: /var/www/icode/ia/phpqueryia.py
...
def buscar_intranet(pergunta):
    ...
    for url in urls:
        ...
        try:
            ...
            log_debug(f"Erro intranet: {str(e)}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 🔹 2) Fallback → RAG vetorial
        embedding = gerar_embedding(pergunta)
        contexto = buscar_contextos(embedding)

        if not contexto.strip():
            sys.stdout.write("Nenhum contexto encontrado.")
            sys.stdout.flush()
            sys.exit(0)
        ...
        sys.stdout.write("Erro interno da IA.")
        sys.stdout.flush()
        sys.exit(1)
