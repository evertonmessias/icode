...
require_once dirname(__FILE__).'/../extractors/extractors.php';
...
class WPFTS_Core 
{
	...
	public function createQLog($query, $params = array())
	{
		global $wpdb;
	...
	*/
	...
	public function getCachedAttachmentContent($post_id, $is_reset_cache = false) 
	{
		...
		if ($post->post_type == 'attachment') {
			...
			// Check if we have valid cached record
			$q = 'select * from `'.$idx.'rawcache` where (`object_id` = "'.addslashes($post_id).'") and (`object_type` = "wp_post")';
			$res = $wpdb->get_results($q, ARRAY_A);

			if ((count($res) < 1) || ($res[0]['cached_dt'] != $post->post_modified) || ($is_reset_cache)) {
				// Cache update required
				$fn = trim(get_post_meta($post_id, '_wp_attached_file', true));
//file_put_contents(dirname(__FILE__).'/debug.txt', print_r($fn, true)."\n", FILE_APPEND);
				$up = wp_upload_dir();
				$up_dir = isset($up['basedir']) ? $up['basedir'].'/' : get_home_path().'/wp-content/uploads/';
				$filename = (strlen($fn) > 0) ? $up_dir.$fn : false;
//file_put_contents(dirname(__FILE__).'/debug.txt', print_r($up, true)."\n", FILE_APPEND);
//file_put_contents(dirname(__FILE__).'/debug.txt', print_r($filename, true)."\n", FILE_APPEND);
...
//exit();
				...
				$filedata = apply_filters('wpfts_attachment_filename', $filedata, $post);

				$v = array();
				if ($filedata && isset($filedata['filename']) && is_file($filedata['filename'])) {

					$filename = $filedata['filename'];
					$mime_type = isset($filedata['mime_type']) ? $filedata['mime_type'] : false;

					$v = $this->index_extract_text($filename, $mime_type);
				} else {
					$v = array(
						'error' => 'No file exists: '.($filedata && isset($filedata['filename']) ? $filedata['filename'] : 'No filename given!'),
						'content' => '',
					);
				}
				$chunks['post_content'] = $v['content'];
	...
	public function getPostChunks($post_id, $is_refresh_raw_cache = false)
	{
		...
		// A smart startup
		do_action('wpfts_index_post_start', $this, $post);
		...
		if ($post->post_type == 'attachment') {
			...
			$att_chunks = $this->getCachedAttachmentContent($post_id, $is_refresh_raw_cache);

			$att_chunks = apply_filters('wpfts_index_attachment', $att_chunks, $post);

			$chunks['attachments_content'] = $att_chunks['post_content'];

		} else {
			if ($this->get_option('include_attachments') != 0) {

				// Get attachments
				$p2 = get_posts(array('post_type' => 'attachment', 'post_status' => 'inherit', 'post_parent' => $post_id));
		
				if (count($p2) > 0) {
					// Index attachments also
					$sum = '';
					foreach ($p2 as $tp) {
						$extr = $this->getCachedAttachmentContent($tp->ID, $is_refresh_raw_cache);

						$extr = apply_filters('wpfts_index_attachment', $extr, $tp);

						if (isset($extr['post_title']) && (strlen($extr['post_title']) > 0)) {
							$sum .= $extr['post_title']."\n\n";
						}

						if (isset($tp->post_excerpt) && (strlen($tp->post_excerpt) > 0)) {
							$sum .= $tp->post_excerpt."\n\n";
						}

						if (isset($tp->post_content) && (strlen($tp->post_content) > 0)) {
							$sum .= $tp->post_content."\n\n";
						}

						if (isset($extr['post_content']) && (strlen($extr['post_content']) > 0)) {
							$sum .= $extr['post_content']."\n\n";
						}
					}
					if (strlen($sum) > 0) {
						$chunks['attachments_content'] = $sum;
					}
				}
			}
		}
	...
	public function EncodePOSTBody($data = array(), $files = array())
	{
		$res = array(
			'headers' => array(),
			'body' => '',
		...
		$mb = '--------------------------'.str_replace(',', '.', microtime(true));
	
		$res['headers']['Content-Type'] = 'multipart/form-data; boundary='.$mb;
		...
		foreach ($files as $k => $filename) {
			if (file_exists($filename)) {
				...
				// File not found!
				return array(
					'error' => sprintf(__('Error: attachment file "%s" do not exist', 'fulltext-search'), $filename),
				);
			}
		}
	...
	public function MakePOSTFileContext($data = array(), $files = array())
	{
		$enc_body = $this->EncodePOSTBody($data, $files);

		if (isset($enc_body['error'])) {
			return $enc_body;
		}
		...
		$context = stream_context_create(array(
			'http' => array(
				'method' => 'POST',
				'header' => $this->MergeHeaders($enc_body['headers']),
				'content' => $enc_body['body'],
			)
		));
	...
	public function try_extractor($e_name, $filepath, $mimetype) 
	{
		$extractors = $this->extractors;

		if (isset($extractors[$e_name])) {
			$tm = $extractors[$e_name];
			if (isset($tm['is_ok']) && ($tm['is_ok'] != 0)) {
				if (isset($tm['mimetypes'][$mimetype])) {
					$p_mime = $tm['mimetypes'][$mimetype];
					if (($p_mime[0] == 0) || ($p_mime[0] == 1)) {
						// Supported!
						if (function_exists($tm['execute'])) {
							$func = $tm['execute'];
							$ret = $func($filepath, $mimetype);
	...
	public function index_extract_text($filepath, $mimetype) 
	{
		$extraction_engine = intval($this->get_option('extraction_engine'));	// 0 = Native, 1 = TextMill

		$ret = array('content' => '', 'error' => '', 'method' => '');
		
		if (file_exists($filepath) && is_file($filepath)) {
			
			// Select the correct engine to parse the file
			if ($extraction_engine == 1) {
				
				// Textmill.io enabled, try it first
				$t_ret = $this->try_extractor('textmill', $filepath, $mimetype);

				if ($t_ret) {
					return $t_ret;
				}

				// If not success with TextMill.io, try Native PHP
			}
			// Only Native PHP allowed
			$t_ret = $this->try_extractor('native-php', $filepath, $mimetype);

			if ($t_ret) {
				return $t_ret;
			}

			$ret['error'] = 'No extractor found for mimetype "'.$mimetype.'"';
		} else {
			// No file presents
	...
	public function ajax_reindex_attachment_post()
	{
		$t0 = microtime(true);
		...
		if (($data = $jx->getData()) !== false) {
			...
			if ($post_id > 0) {
				/*
				$success = $this->reindex_post($post_id, false, true);
				if ($success) {
					// Fine
					$jx->variable('code', 0);
				} else {
					// Error
					$error = $this->index_error;

					$jx->variable('code', 0);
					$jx->variable('message', __("An error occured when indexing the post", 'fulltext-search').":\n\n".$error);
				}*/

				wpfts_post_reindex($post_id, false, true);

				$jx->variable('code', 0);

				// Output attachment info window also
				$jx->variable('html', wpfts_get_extracted_content_block($post_id));
			} else {
				$jx->variable('code', 0);
				$jx->variable('message', __("An error occured when indexing the post ID=".$post_id, 'fulltext-search'));
			}
					...
					if (preg_match('~^eclustertype_(.+)$~', $k, $m)) {
						...
						switch ($key) {
							...
							case 'display_attachments':
								...
								$this->set_option($key, $v);
								break;
							case 'limit_mimetypes':
								$this->set_option($key, trim($d));
								break;
		...
		if (($data = $jx->getData()) !== false) {
			...
			if (wp_verify_nonce($data['wpfts_options-nonce'], 'wpfts_options')) {
						...
						switch ($key) {
							case 'maxrepeat':
								...
								if (is_numeric($d) && ($v >= 0) && ($v <= 100)) {
									...
									$e[] = array($key, __('Maximum Word Frequency should be an integer number from 0 to 100', 'fulltext-search'));
								}
								break;
							case 'minlen':
								$v = intval($d);
								if (is_numeric($d) && ($v >= 0) && ($v <= 50)) {
									...
									$e[] = array($key, __('Minimum Word Length should be an integer number from 0 to 50', 'fulltext-search'));
								}
								break;
							case 'stopwords':
								$v = trim($d);
				...
				if (count($e) > 0) {
					...
					if (isset($data['wpfts_is_autoreindex']) && ($data['wpfts_is_autoreindex'] == 1)) {
	 ...
	 * Save Indexing Engine Settings
	 */
	 public function ajax_submit_settings4() 
	 {
		$jx = new WPFTS_jxResponse();
		...
		{
			// Check nonce
			if (wp_verify_nonce($data['wpfts_options-nonce_licensing'], 'wpfts_options_licensing')) 
			{
				$e = array();

				$email = isset($data['email']) ? trim($data['email']) : '';

				WPFTS_Updater::save_subscription_email($email);

				if (strlen($email) > 0) {
					$jx->alert(__('Your license key has been successfully stored.', 'fulltext-search'));
				} else {
					$jx->alert(__('Your license key was removed. '."\n\n".'WARNING: Your automatic updates has been disabled.', 'fulltext-search'));
				}
				$jx->reload();
			} else {
				$jx->alert(__('The form is outdated. Please refresh the page and try again.', 'fulltext-search'));
	...
	{	
		...
		do_action('wpfts_extractors_init');
	}
		...
		if (!$is_wpfts_disabled) {
			...
			if (($wpq->is_main_query() && !is_admin()) || ($is_force)) {
				...
				if ((strlen($wgt) > 0) && (isset($ws[$wgt]))) {
					...
					if ($display_attachments && ($wpq->is_main_query() && !is_admin())) {
						if ($wpq->is_search()) {
							...
							if ($post_types && ($post_types != 'any')) {
								if (is_array($post_types)) {
									...
									if ($post_types != 'attachment') {
										$post_types = array($post_types, 'attachment');
									}
								}
							} else {
								$in_search_post_types = get_post_types( array('exclude_from_search' => false) );
								$post_types = array_merge(array('attachment'), $in_search_post_types);
							}
							$wpq->set('post_type', $post_types);
							$post_status = $wpq->get('post_status');
							if ($post_status) {
								if (is_array($post_status)) {
									...
									if ($post_status != 'inherit') {
										$post_status = array($post_status, 'inherit');
									}
								}
							} else {
								$post_status = array('inherit', 'publish');
							}
							$wpq->set('post_status', $post_status);
						}
					}
				}
	...
	public function process_form_indexingbox($jx, $data)
	{
		if (wp_verify_nonce($data['wpfts_options-nonce_indexingbox'], 'wpfts_options_indexingbox')) {
			...
			$include_attachments = isset($data['wpfts_include_attachments']) ? $data['wpfts_include_attachments'] : 0;
			...
			$this->set_option('include_attachments', $include_attachments ? 1 : 0);
	...
	public function process_form_extractionbox($jx, $data)
	{
		if (wp_verify_nonce($data['wpfts_options-nonce_extractionbox'], 'wpfts_options_extractionbox')) {
			...
			$extraction_engine = isset($data['wpfts_extraction_engine']) ? intval($data['wpfts_extraction_engine']) : 0;

			$this->set_option('extraction_engine', $extraction_engine);
	...
	public function process_form_step2_find_records($jx, $data)
	{
		if (wp_verify_nonce($data['wpfts_options-nonce_step2_find_records'], 'wpfts_options_step2_find_records')) {
			...
			$display_attachments = isset($data['wpfts_display_attachments']) ? intval($data['wpfts_display_attachments']) : 0;
			$limit_mimetypes = isset($data['wpfts_limit_mimetypes']) ? trim($data['wpfts_limit_mimetypes']) : '';

			$this->set_option('deflogic', $deflogic ? 1 : 0);
			$this->set_option('deeper_search', $deeper_search ? 1 : 0);
			$this->set_option('display_attachments', $display_attachments ? 1 : 0);
			$this->set_option('limit_mimetypes', $limit_mimetypes);
			...
			if ($form_name) {
				...
				switch ($form_name) {
					case 'form_controlbox':
						$this->process_form_controlbox($jx, $data);
						break;
					case 'form_indexingbox':
						$this->process_form_indexingbox($jx, $data);
						break;
					case 'form_extractionbox':
						$this->process_form_extractionbox($jx, $data);
						break;
					case 'form_step1_query_preprocessing':
						$this->process_form_step1_query_preprocessing($jx, $data);
						break;
					case 'form_step2_find_records':
						$this->process_form_step2_find_records($jx, $data);
						break;
					case 'form_step3_calculate_relevance':
						$this->process_form_step3_calculate_relevance($jx, $data);
						break;
					case 'form_step4_sort_results':
						$this->process_form_step4_sort_results($jx, $data);
						break;
					case 'form_step5_show_results':
						$this->process_form_step5_show_results($jx, $data);
						break;
					default:
