...
class WPFTS_Output
{
	...
	public function useful_box($post) 
	{
		...
		<div class="card mb-2">
			...
			<div class="card-body">
				...
				<hr>
				<p>Do you need a feature? <a href="<?php echo $wpfts_core->_wpfts_domain.'/contact/'; ?>">Request here.</a></p>
			</div>
		</div>
		<?php
	...
	public function control_box($post = null) 
	{
		...
		ob_start();
		?>
		<div class="card mb-2 mt-2 wpfts_smartform" data-name="form_controlbox">
		...
		?><div class="mb-2 mt-2 wpfts_smartform" data-name="form_step2_find_records">
			...
			<div class="bg-light">
				...
				<div class="row">
					...
					<div class="col fixed-150">
						<?php
						$disat = intval($wpfts_core->get_option('display_attachments'));
						echo WPFTS_Htmltools::makeLabelledCheckbox('wpfts_display_attachments', 1, __('Enabled', 'fulltext-search'), $disat);
						?>
					</div>	
					<div class="col d-xl-none text-right">
				...
				$limit_mimetypes = trim($wpfts_core->get_option('limit_mimetypes'));
				$listed_mt = array();
				$lm_a = strlen($limit_mimetypes) > 0 ? explode(',', $limit_mimetypes) : array();
				foreach ($lm_a as $d) {
					$listed_mt[trim($d)] = 1;
				}
				...
				$render_group = function($group_id, $mt_stat) use ($mtg, $listed_mt) 
				{
					if (isset($mt_stat[$group_id]) && (count($mt_stat[$group_id]) > 0)) {
						...
						<tr>
							...
							?>
							<div class="wpfts_mt_item"><label for="<?php echo $key; ?>" title="<?php echo htmlspecialchars($d); ?>"><input type="checkbox" name="<?php echo htmlspecialchars($key); ?>" value="<?php echo htmlspecialchars($d); ?>" id="<?php echo $key; ?>" <?php echo isset($listed_mt[$d]) ? ' checked="checked"' : ''; ?>> <?php echo htmlspecialchars($key_name); if ($nn > 0) { echo ' ('.$nn.')'; } ?></label></div>
							<?php
							}
							?>
							</td>
			...
			?><div class="ft_limit_filetypes">
				<div class="row">
					...
					<div class="col fixed-150">
						<div style="padding: 0px 0px 15px 0px;"><label for="ft_mt_all"><input type="checkbox" name="ft_mt_all" id="ft_mt_all" value="1" <?php echo count($listed_mt) > 0 ? '' : ' checked="checked"'; ?>> Allow All</label></div>
						<?php
						echo WPFTS_Htmltools::makeHidden($limit_mimetypes, array('name' => 'wpfts_limit_mimetypes', 'style' => 'width:100%;'));
						?>
					</div>
					<?php /*
					<div class="col d-xl-none text-right">
						<p><a data-toggle="collapse" href="#wf_hint_limitft" role="button" aria-expanded="false" aria-controls="wf_hint_limitft"><i class="fa fa-info-circle"></i></a></p>
					</div>
					<div class="col col-xl col-12 d-xl-block collapse" id="wf_hint_limitft">
						<p class="text-secondary"><i><?php echo __('When checked on, WPFTS will search attachments by contents and show them in search results like usual posts.', 'fulltext-search'); ?></i></p>
					</div>
					*/ ?>
				</div>
				...
				<div class="row mt-3-sm">
					...
					<div class="col fixed-250">
						<ul>
							...
							<li><?php
								$is_file_ext = intval($wpfts_core->get_option('is_file_ext'));
								echo WPFTS_Htmltools::makeLabelledCheckbox('wpfts_is_file_ext', 1, __('File Extension', 'fulltext-search'), $is_file_ext);
								?>
							</li>
							<li><?php
								$is_filesize = intval($wpfts_core->get_option('is_filesize'));
								echo WPFTS_Htmltools::makeLabelledCheckbox('wpfts_is_filesize', 1, __('Filesize', 'fulltext-search'), $is_filesize);
								?>
							</li>
							<li><?php
								$is_direct_link = intval($wpfts_core->get_option('is_direct_link'));
								echo WPFTS_Htmltools::makeLabelledCheckbox('wpfts_is_direct_link', 1, __('Direct Download Link', 'fulltext-search'), $is_direct_link);
								?>
							</li>
							<li><?php
								$is_title_direct_link = intval($wpfts_core->get_option('is_title_direct_link'));
								echo WPFTS_Htmltools::makeLabelledCheckbox('wpfts_is_title_direct_link', 1, __('Link Title to File Download', 'fulltext-search'), $is_title_direct_link);
								?>
							</li>
						</ul>
					</div>
	...
	public function indexing_box($post) 
	{
		...
		<div class="wpfts_smartform bg-light" data-name="form_indexingbox">
			...
			<div>
				...
				<div class="row">
					<div class="col fixed-200 font-weight-bolder">
						<?php echo __('Include Attachments', 'fulltext-search'); ?>
					</div>
					<div class="col fixed-150">
						<?php
						$include_attachments = intval($wpfts_core->get_option('include_attachments'));
						echo WPFTS_Htmltools::makeLabelledCheckbox('wpfts_include_attachments', 1, __('Enabled', 'fulltext-search'), $include_attachments);
						?>
					</div>	
					<div class="col d-xl-none text-right">
	...
	public function extraction_box($post)
	{
		...
		$extractors = $wpfts_core->extractors;

		$used_mt = $wpfts_core->GetUsedMimetypes();

		// let's create a table of extensions
		$extlist = array();
		foreach ($extractors as $k_extr => $v_extr) {
			if (isset($v_extr['mimetypes']) && is_array($v_extr['mimetypes'])) {
				foreach ($v_extr['mimetypes'] as $t_mime => $t_props) {
					$is_used = isset($used_mt[$t_mime]) ? 1 : 0;
					$tt_exts = isset($t_props[1]) ? $t_props[1] : array();
					if (!is_array($tt_exts)) {
						if (strlen($tt_exts) > 0) {
							$tt_exts = array($tt_exts);
						} else {
							$tt_exts = array();
						}
					}

					// Check if we already have this mimetype in list
					if (!isset($extlist[$t_mime])) {
						// Create new mimetype row
						$extlist[$t_mime] = array('is_used' => $is_used, $k_extr => array($t_props[0]), 'ftypes' => $tt_exts);
					} else {
						// Append with new data
						$extlist[$t_mime]['is_used'] = max($extlist[$t_mime]['is_used'], $is_used);
						if (isset($extlist[$t_mime][$k_extr])) {
							// Find better value
							$old_v = $extlist[$t_mime][$k_extr][0];
							if ($t_props[0] >= 0) {
								$extlist[$t_mime][$k_extr] = array(min($t_props[0], $old_v));
							}
						} else {
							$extlist[$t_mime][$k_extr] = array($t_props[0]);
						}
						// Add file types
						foreach ($tt_exts as $t_e) {
							if (!in_array($t_e, $extlist[$t_mime]['ftypes'])) {
								$extlist[$t_mime]['ftypes'][] = $t_e;
							}
						}
					}

					/*
					foreach ($tt_exts as $t3) {
						if (!isset($extlist[$t3])) {
							$extlist[$t3] = array('is_used' => $is_used, $k_extr => array($t_props[0]));
						} else {
							$extlist[$t3]['is_used'] = max($extlist[$t3]['is_used'], $is_used);
							if (isset($extlist[$t3][$k_extr])) {
								// Find better value
								$old_v = $extlist[$t3][$k_extr][0];
								if ($t_props[0] >= 0) {
									$extlist[$t3][$k_extr] = array(min($t_props[0], $old_v));
								}
							} else {
								$extlist[$t3][$k_extr] = array($t_props[0]);
							}
						}
					}
					*/
		...
		$extraction_engine = intval($wpfts_core->get_option('extraction_engine'));	// 0 = Native, 1 = TextMill
		
		$engines = array(
			1 => 'TextMill.io then Native PHP',
			0 => 'Native PHP only',
		);

		$key_order = ($extraction_engine > 0) ? array('textmill', 'native-php') : array('native-php', 'textmill');
			...
			<div class="bg-light">
				...
				<div class="row">
					...
					<div class="col fixed-250">
						<div class="wpfts_search_logic_group">
						<?php
							echo WPFTS_Htmltools::makeRadioGroup('wpfts_extraction_engine', $engines, $extraction_engine, array());
						?>
						</div>
					</div>	
					<div class="col d-xl-none text-right">
				...
				<div class="row">
					<div class="col-12">
						<h5><?php echo __('How my files are processed now?', 'fulltext-search'); ?></h5>
						<div class="">
							<div class="bd-callout bd-callout-secondary bg-white">
						<p><?php echo __('Status explanation:', 'fulltext-search'); ?></p>
		<ul>
			<li><span class="badge badge-secondary"><?php echo __('-', 'fulltext-search'); ?></span> <?php echo __('The extraction engine does not support this file extension yet', 'fulltext-search'); ?></li>
			<li><span class="badge badge-secondary"><?php echo __('OK', 'fulltext-search'); ?></span> <?php echo __('Supported and allowed', 'fulltext-search'); ?></li>
			<li><span class="badge badge-secondary"><?php echo __('LowQ', 'fulltext-search'); ?></span> <?php echo __('Supported, but has low quality for some files', 'fulltext-search'); ?></li>
			<li><span class="badge badge-secondary"><?php echo __('Not Allowed', 'fulltext-search'); ?></span> <?php echo __('Supported, but disabled because of license level restrictions', 'fulltext-search'); ?></li>
			<li><span class="badge badge-secondary"><?php echo __('Disabled', 'fulltext-search'); ?></span> <?php echo __('Disabled via Settings', 'fulltext-search'); ?></li>
		</ul>
		<p><?php echo __('The extraction engine which will be actually used is marked <span class="text-success">green</span>', 'fulltext-search'); ?></p>
							</div>
						</div>
		<?php

		?>
		<table class="table table-sm table-compact table-hover bg-white">
		<thead>
		<tr>
			<th><?php echo __('File Type', 'fulltext-search'); ?></th>
			<?php
			foreach ($key_order as $kk) {
				if (isset($extractors[$kk])) {
					echo '<th>'.htmlspecialchars($extractors[$kk]['title']).'</th>';
				}
			}
			?>
			<th><?php echo __('This Website Has Such Files', 'fulltext-search'); ?></th>
		</tr>
		</thead>
		<tbody>
		<?php

		// Order extensions by is_used and alphabet (improvement required later)
		uksort($extlist, function($k1, $k2) use (&$extlist)
		{
			$v1 = $extlist[$k1];
			$v2 = $extlist[$k2];
			if ($v1['is_used'] < $v2['is_used']) {
				return 1;
			} elseif ($v1['is_used'] > $v2['is_used']) {
				return -1;
			} else {
				if (isset($v1['ftypes'][0]) && isset($v2['ftypes'][0])) {
					return strcmp($v1['ftypes'][0], $v2['ftypes'][0]);
				} else {
					if (isset($v1['ftypes'][0])) {
						return 1;
					} else {
						return -1;
					}
		...
		foreach ($extlist as $k => $d) {

			$is_used = isset($d['is_used']) && ($d['is_used'] > 0);
			$collapse = $is_used ? '' : ' class="collapse wf_show_exts"';
			...
			?>
		<tr<?php echo $collapse; ?>>
			<td><?php 
				$tt = $d['ftypes'];
				usort($tt, function($v1, $v2)
				{
					return strcmp($v1, $v2);
				});
				echo mb_strtoupper(implode(', ', $tt));
			?></td>
			<?php
				$already_used = false;
				foreach ($key_order as $kk) {
				if (isset($extractors[$kk])) {
					...
					$is_good = false;
					if (($kk == 'textmill') && ($extraction_engine == 0)) {
						$s = 'Disabled';
					} else {
						switch ($supp) {
							case -1:
					...
					$is_bold = $is_good && (!$already_used);
					if ($is_bold) {
						$already_used = true;
					}
					if ($is_bold) {
						echo '<td><b><span style="color:green;">';
						echo htmlspecialchars($s);
						echo '</span></b></td>';
					} else {
						echo '<td><span style="color:gray;">';
						echo htmlspecialchars($s);
						echo '</span></td>';
					}
				}
			...
			?>
			<td><?php echo (isset($used_mt[$k]) && ($used_mt[$k] > 0)) ? $used_mt[$k] : ''; ?></td>
		</tr>
			<?php
		}
		?>
		...
		<?php
			if ($n_not_used > 1) {
		?>
		<p class="show_wf_show_exts"><?php echo sprintf(__('There are <b>%s</b> more extensions which is not yet used by this website.', 'fulltext-search'), $n_not_used); ?> <a href="#" class="btn_wf_show_exts"><?php echo __('Show them', 'fulltext-search'); ?></a></p>
		<script type="text/javascript">
		jQuery(document).ready(function()
		{
			jQuery('.btn_wf_show_exts').on('click', function(e)
			{
				e.preventDefault();

				jQuery('.show_wf_show_exts').toggle();
				jQuery('.wf_show_exts').toggle();
