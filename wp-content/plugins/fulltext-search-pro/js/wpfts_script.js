...
function wpftsShowIndexStatus(st) 
{
	...
	jQuery('.wpfts_data_isdisabled', box).css('display', st['enabled'] ? 'none' : 'inline-block');
	jQuery('.wpfts_data_isindexready', box).css('display', (st['enabled'] && st['index_ready']) ? 'inline-block' : 'none');
	jQuery('.wpfts_data_isindexready_not', box).css('display', (st['enabled'] && (!st['index_ready'])) ? 'inline-block' : 'none');

	jQuery('.wpfts_data_n_inindex', box).html('' + st['n_inindex']);

	var is_ok = false;
	var is_slow_warning = false;
	...
	if (st['autoreindex']) {
		...
		if (parseFloat(st['n_inindex']) > 0) {
			percent = (0.0 + parseInt(st['n_actual'])) * 100 / (parseFloat(st['n_inindex']));
		} else {
			percent = 0;
		}

		percent2 = st['nw_act'] * 100 / st['nw_total'];

		if ((st['n_pending'] > 0) || (st['n_tw'] > 0)) {
			// Main indexing mode
			is_indexing = true;
		} else {
			is_indexing = false;
			if (st['nw_act'] < st['nw_total']) {
	...
	jQuery('.wpfts_data_percent2', box).attr('aria-valuenow', percent2).css('width', '' + percent2 + '%').html(percent2.toFixed(2) + '%');
