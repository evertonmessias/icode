...
/*
Plugin Name: WP Fulltext Search Pro
Description: Implementing a true indexed full-text search over wordpress posts and metas without using any external indexing software. Pro version.
Version: 2.49.192
Tested up to: 5.9.1
Author: Epsiloncool
 ...
 * All rights reserved.
 * 
 * Full license text in /classes/pdftotext/ subdirectory
 */
...
//	wp_enqueue_style('', plugins_url('style/wpfts_main.css', __FILE__), array(), $version);
	wp_enqueue_style('wpfts_jquery-ui-styles', $wpfts_core->root_url.'/style/wpfts_autocomplete.css', array(), $version);
	wp_enqueue_script('wpfts_frontend', plugins_url('js/wpfts_frontend.js', __FILE__), array('jquery'), $version);
});
...
add_action('init', function () 
{
	...
	if ((is_object($wpfts_core)) && (is_callable(array($wpfts_core, 'set_hooks')))) {
		...
		if (is_admin()) {
			...
			add_action('wp_ajax_wpftsi_submit_settings4', array($wpfts_core, 'ajax_submit_settings4'));
			...
			add_action('wp_ajax_wpftsi_reindex_attachment_post', array($wpfts_core, 'ajax_reindex_attachment_post'));
		}
...
function wpfts_plugin_links($links, $file)
{
	if (basename($file) == basename(__FILE__)) {
		// Check if we already have View Details link
		$is_view_details = false;
		foreach ($links as $link)
		{
			if (preg_match('~plugin-install\.php\?tab=plugin\-information~', $link)) {
				$is_view_details = true;
			}
		}
		if (!$is_view_details) {
			$links[] = '<a href="plugin-install.php?tab=plugin-information&amp;plugin=fulltext-search-pro&amp;TB_iframe=true&amp;width=772&amp;height=868" class="thickbox open-plugin-details-modal" aria-label="More information about WP Fulltext Search Pro" data-title="WP Fulltext Search Pro">'.__('View Details', 'fulltext-search').'</a>';
		}
...
function wpfts_admin_menu()
{
	...
	$menu_items = array(
		...
		'wpfts-options-licensing' => array(__('Licensing', 'fulltext-search')),
	);
...
function wpfts_enqueues($hook_suffix)
{
	...
	wp_enqueue_script('wpfts_script_fs', plugins_url('js/wpfts_script_fs.js', __FILE__), array(), $version);
...
add_filter('the_title', function($out)
{
	...
	if ($wpfts_core && is_a($wpfts_core, 'WPFTS_Core')) {
		...
		if ($is_smart_excerpts != 0) {
			if ((is_search() && !is_admin() && in_the_loop()) || ($wpfts_core->forced_se_query !== false)) {
				$post_id = get_the_ID();
...
/** Pro features **/
function wpfts_edit_attachment_action($post_id)
{
	//file_put_contents(dirname(__FILE__).'/ilog.txt', 'Edit attachment: '.print_r($post_id, true)."\n", FILE_APPEND);

	wpfts_post_reindex($post_id);
}
add_action('edit_attachment', 'wpfts_edit_attachment_action', 99);

function wpfts_add_attachment_action($post_id)
{
	//file_put_contents(dirname(__FILE__).'/ilog.txt', 'Add attachment: '.print_r($post_id, true)."\n", FILE_APPEND);

	wpfts_post_reindex($post_id);
}
add_action('add_attachment', 'wpfts_add_attachment_action', 99);

function wpfts_delete_attachment_action($post_id)
{
	//file_put_contents(dirname(__FILE__).'/ilog.txt', 'Delete attachment: '.print_r($post_id, true)."\n", FILE_APPEND);

	wpfts_post_reindex($post_id, true);	// Force remove
}
add_action('delete_attachment', 'wpfts_delete_attachment_action', 99);

function wpfts_extract_text($filepath, $mimetype)
{
	global $wpfts_core;
	
	return $wpfts_core->index_extract_text($filepath, $mimetype);
}

require dirname(__FILE__).'/includes/updater/updater.php';
